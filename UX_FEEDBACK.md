...
- `[ ]` **[M] Likes/dislikes not visible or working** in the player — **this is a bug**, not
...
- `[ ]` **[S] Subscription name → link** to that subscription's page (from the player).
- `[ ]` **[S] "Mark for deletion" button** in the player. _Ties into the existing scheduled-
  deletion flow (`ProcessScheduledDeletionsJob` / `Video.DownloadSkipped` family)._
- `[ ]` **[M] "Up next" should list downloaded videos first**, and each should show its
  badges (same badge set as the video list).
- `[ ]` **[M] Show download progress in the player.** If a video is downloading, show that
