...
- `[~]` **[S/M] Description formatting + typography.** Linkify URLs/@handles/#tags and make
  ...
  **Typography done in Batch 1** (`_watch.scss`: 70ch max-width, line-height 1.65, secondary
  colour, link styling — verified live). **Linkify/markdown/seekable timestamps remain (Batch 4).**
- `[ ]` **[L] Subtitles in the player** with language selection, labelling
  autogenerated vs human. _We already download subs; need to expose the sidecar tracks to
  the video element as `<track>`s + a picker._
...
**Batch 4b — the two that need design**: subtitles in the player, and description
linkify/markdown/seekable timestamps.
